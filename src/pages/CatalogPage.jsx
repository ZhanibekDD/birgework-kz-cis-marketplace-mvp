...
export default function CatalogPage() {
  ...
  return (
    <div className="container-main py-8">
      ...
      <p className="section-desc mt-2">{filtered.length} услуг по вашему запросу</p>
      <div className="card-surface mt-5 grid gap-3 p-4 md:grid-cols-4">
        <label className="sr-only" htmlFor="catalog-search">Поиск</label>
        <input id="catalog-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Поиск услуги" className="rounded-xl bg-slate-100 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-300" />
      ...
      {filtered.length === 0 ? (
        <div className="card-surface mt-6 p-8 text-center">
          ...
          <p className="text-sm text-slate-500">Измените фильтры или поисковый запрос.</p>
        </div>
      ) : (
